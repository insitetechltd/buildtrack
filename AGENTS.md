...
- Current pipeline focus precedence: (1) **`M-OPS-ENV-01` Prod vs Dev Supabase** (empty PROD first — Order 14.94; plan `docs/superpowers/plans/2026-08-26-prod-dev-supabase-split.md`; daily TF→DEV; Stripe live @ App Store submit). (2) **M-AUTHZ-RC** + finish **M-BILL-01** on DEV. (3) **M-DATA-04** dampened (optional formal close). (4) App Store → live Stripe on PROD. (5) **M-OPS-03** Owner management interface + **M-AUTHZ-02** → **M-AI-01** / Wave 2. **Customer-managed storage (`WS-STORAGE`) Deferred.** Maestro/CI against **DEV only** until PROD is intentional.
...
- `WS-AUTHZ / M-AUTHZ-RC` **RC BLOCKER (2026-08-24):** Construct locked — CA / PM / Worker + PA (crown on CA\|PM only); drop title picker; same field UI + avatar for CA. SoT: `docs/superpowers/plans/2026-08-24-company-user-project-model.md`. Runtime catch-up remaining. Not `M-AUTHZ-02`.
