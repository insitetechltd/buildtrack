...
export function RealtimeSyncManager() {
  ...
  useEffect(() => {
    ...
    const handleSubscriptionError = (channelName: string, error: any) => {
      console.error(`❌ [Realtime] ${channelName} subscription error:`, error);
    ...
    const tasksChannel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          handleSubscriptionError('tasks', 'Channel error');
        }
      });
    ...
    const taskUpdatesChannel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          handleSubscriptionError('task_updates', 'Channel error');
        }
      });
    ...
    const projectsChannel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          handleSubscriptionError('projects', 'Channel error');
        }
      });
    ...
    const usersChannel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          handleSubscriptionError('users', 'Channel error');
        }
      });
