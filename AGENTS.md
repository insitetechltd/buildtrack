...
- `WS-FIELD / M-REPORT-01` **Pipeline (opened 2026-09-23)**: Report resolve-**with-reply** audit trail — close + typed reply (+ photos) must persist `resolved` + `issue_resolved` (actor/note) **and** timeline-visible reply on same task id. Stage E **E3c OPEN** (E3 without-reply Closed; E3b promote OPEN). Must not jump App Store spine. Do not let `S-UX-01R` Phase B erase Report reply audit.
