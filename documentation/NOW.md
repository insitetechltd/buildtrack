...
**This session — CBP resume STOP (2026-09-17):** Gate 0 freeze ancestry **OK** (`fa5cff1` ⊂ HEAD `6a7c127` on `cursor/freeze-dual-path-phase-c-5b19`). Build# bumped **262→263** via `build-local.sh`. Local IPA **FAILED** at Prepare credentials: dist cert fingerprint `52CF5B2C…` **hasn't been imported successfully** (same class as TF 260/261 local fails). Hard law: **no cloud `eas build` fallback**. Hygiene: pruned `.eas/local-build` ~221G→~4G; cleaned stale `eas-build-*.keychain` search entries; aborted cloud build none (262 already finished). **No TF submit.** Next human: unlock login keychain / interactive `EAS_ALLOW_INTERACTIVE=1 ./build-local.sh ios production true` → approve Keychain prompts → verify `security find-identity -v -p codesigning` shows Dist identity → then `eas submit` local IPA. Report: Mac project-store `docs/cbp-tf-freeze-2026-09-17.md`.
