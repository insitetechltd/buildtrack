...
**This session — iPad tiles 3:4 cover (2026-09-20):** Field photos are mostly phone-portrait. Equal 3-up / 4-up cells stay, but aspect is **3:4** with `cover` (no letterbox). Landscape shots crop on the sides; tap still opens uncropped gallery. Phone square hero unchanged. Not in TF yet (277 still has 4:3 contain). **Next:** headed iPad smoke, CBP if asked.

**This session — CBP TF 277 submitted (2026-09-20):** Gate 0 waived (user-ordered CBP of iPad Task Detail mix; Metro→PROD GO on `fa5cff1`; this SHA is JS layout only — no persistence/auth writes). App `7e1f215` (portrait stacked info card; landscape meta left ~1/3; fixed 3-up/4-up `contain` tiles + peek-to-slide; phone square hero unchanged). Prove: layout **6/6**, timeline **13/13**, sticky-layout **23/23**, TaskDetailAcceptanceUI **19/19**; `npx tsc --noEmit` rc=0. Local `./build-local.sh ios production` → IPA **v1.1.3 (277)** `.eas/artifacts/build-1789916110248.ipa` (CFBundleVersion 277; bundle `com.buildtrack.app.local`; PROD host `jcnzjigxgkzhjsaekoqz`; DEV host absent; bake includes `isIpadLandscapeMetaSplit`). EAS submit https://expo.dev/accounts/insitetech/projects/buildtrack/submissions/0679fff2-e856-47d8-87c5-65b107eb9ff5 (✔ Uploaded ASC; Apple processing). ASC Public / Submit for Review untouched. **Next:** dogfood TF **277** on iPad — portrait meta on top + 3-up tiles; rotate → meta left + 4-up.

**This session — iPad Task Detail mix GO (2026-09-20):** Portrait keeps the stacked info card; landscape splits project/task meta left (~1/3) and the thread right. Timeline chrome unchanged. iPad photos are a **fixed tile token** (portrait 3-up, landscape 4-up; 1/2/3 or 1–4 fill left-aligned; `contain`; >N horizontal peek-to-slide; no +N). Phone square hero unchanged. Prove: layout **6/6**, timeline **13/13**, sticky-layout **23/23**, TaskDetailAcceptanceUI **19/19**; `npx tsc --noEmit` rc=0. Gate B: composer-2.5 ITERATE on headed smoke (no sim booted). **Next:** superseded for iPad dogfood by TF **277**.
