...
- `WS-SUPABASE / M-SUPABASE-04c` **Closed (2026-08-13)** docs/policy: paying tenants keep files indefinitely on hot `buildtrack-files`; after plan expiry restore = back-pay up to 6 months (objects stay hot). **No live Dashboard Expire, no bucket-wide expire, no SQL migrations.** Cold archive parked as 04e. Evidence: `documentation/audit/database/SUPABASE_OPERATIONS_RUNBOOK.md` § Storage retention + ROADMAP Notes + `docs/superpowers/reports/2026-08-13-m-supabase-04c-close.md`.
...
- `WS-AUTHZ / M-AUTHZ-RC` **Closed (2026-08-27):** Host-company CA/PM/Worker + PA (crown on CA\|PM only); field UI + avatar Company management; H01–H08 headed PASS. SoT: `docs/superpowers/plans/2026-08-24-company-user-project-model.md`. Close: `docs/superpowers/reports/2026-08-27-m-authz-rc-close.md`. Not `M-AUTHZ-02`.
- `WS-AUTHZ / M-AUTHZ-02` **Pipeline (locked 2026-08-21)**: Multi-company project membership — partner liaison + project-scoped invite + optional host-absorb seats. **Post-RC.** Canonical: `documentation/multi-company-project-membership.md`. Clarification addendum in `docs/superpowers/analysis/2026-08-19-roadmap-clarification.md`.
- `WS-DATA / M-DATA-04` **Closed (2026-08-27):** Realtime subscribe/reconnect storm dampened; idle + Dashboard SQL gate PASS (`list_changes` ~218/~1.2s; `sub_tables` 28; no INSERT flood). Close: `docs/superpowers/reports/2026-08-27-m-data-04-close.md`.
- `captureSession` **Closed (2026-08-27):** Hybrid camera/library production SoT (Camera tab + Create/Update Add Photos); headed Create+Update smoke PASS. Close: `docs/superpowers/reports/2026-08-27-capture-session-close.md`. Guide: `docs/superpowers/plans/2026-08-27-capture-session-module-ab.md`.
...
- `WS-PERF / M-PERF-04` **Pipeline (opened 2026-09-05)**: Field write-path performance — Create Task / Update Progress / photo upload latency intolerably slow in dogfood. Instrument → ranked bottlenecks → smallest fix slices. Distinct from M-PERF-01 (display) / M-PERF-03 (picker) / M-PERF-02 (upload bytes). Gate A idle-parallel; must not jump App Store spine.
...
- `WS-AI / M-AI-02` **Pipeline (2026-08-19)**: Drawing **assist** only (title block, current rev, cite a crop). Not infer geometry. Prereq document register (M-DMS-01). PDF first; CAD via convert or specialist API — not an in-app DWG parser. Superseded-rev answers = safety defect.
- `WS-AI / M-AI-03` **Pipeline (2026-08-19)**: Photo package/fastener → spec-check for this location/sequence. Barcode vs approved submittal (M-DMS-04). Unlabeled hardware must not auto-pass.
- `WS-COST / M-COST-01` **Pipeline**: Jobsite cost after DMS. Not R6 paper / not GB metering.
