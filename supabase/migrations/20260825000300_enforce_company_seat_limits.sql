...
BEGIN
  ...
  FOR v_row IN
    SELECT u.role, COALESCE(u.is_active, true) AS is_active
    FROM public.users u
    WHERE u.company_id = v_company_id
      AND u.id IS DISTINCT FROM NEW.id
  LOOP
    SELECT * INTO v_contrib
    FROM public.user_seat_contribution(v_row.role, v_row.is_active);
...
CREATE TRIGGER trg_users_enforce_company_seat_limits
  BEFORE INSERT OR UPDATE OF role, is_active, company_id
  ON public.users
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_company_seat_limits();

COMMENT ON FUNCTION public.enforce_company_seat_limits() IS
  'Hard seat gate: reject INSERT/UPDATE that would exceed company_entitlements PM/worker limits.';
