...
- `WS-SUPABASE / M-SUPABASE-03a` **Phase A artefacts (2026-08-09) — BLOCKED awaiting GO**: `supabase/migrations/20260809000100_msupabase03a_role_column_integrity.sql` + `docs/superpowers/reports/2026-08-09-m-supabase-03a-phase-a-schema-review.md`. **NO live apply** until exact phrase `you have GO for M-SUPABASE-03a live apply`.
- `WS-SUPABASE / M-SUPABASE-03c` **Phase A artefacts (2026-08-09) — BLOCKED awaiting GO**: `supabase/migrations/20260809000200_msupabase03c_storage_bucket_policy.sql` + `docs/superpowers/reports/2026-08-09-m-supabase-03c-phase-a-storage-review.md` + runbook note. Signed-URL client cutover deferred. **NO live apply** until exact phrase `you have GO for M-SUPABASE-03c live apply`.
...
- `WS-SUPABASE / M-SUPABASE-04a` **App reconnect shipped (2026-08-10)**; live publication membership audit pending (no `~/.pgpass`). Artefacts: `RealtimeSyncManager` backoff + `docs/superpowers/sql/20260810_msupabase04a_publication_membership_audit.sql` + report `2026-08-10-m-supabase-04a-phase-a.md`.
- `WS-SUPABASE / M-SUPABASE-04b` **Blocked until ~2026-09-07** (30d cool-down post 03b 2026-08-08). No column drops.
- `WS-SUPABASE / M-SUPABASE-04c` **Blocked on 03c live GO**. Retention stub in `SUPABASE_OPERATIONS_RUNBOOK.md` only.
...
- Current pipeline focus precedence: (1) Human GO for 03a/03c live apply → (2) finish 04a live publication audit + optional 04d GO → (3) 04b after ~2026-09-07. `S-UX-01P` is Pipeline but **deferred improvement** — not active focus.
