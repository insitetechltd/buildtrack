...
**This session — CBP resume STOP (2026-09-17):** Gate 0 freeze ancestry **OK** (`fa5cff1` ⊂ HEAD `18ef2a2` on `cursor/freeze-dual-path-phase-c-5b19`). Build# **263**. Local IPA blocked: **0 valid codesigning identities** (same Dist/keychain class as TF 260/261). **No cloud fallback. No TF submit.** Next human: unlock login keychain → `EAS_ALLOW_INTERACTIVE=1 ./build-local.sh ios production true` → Dist identity in `security find-identity -v -p codesigning` → `eas submit` that local IPA.
