...
- Current pipeline focus precedence: (1) **M-DATA-04** dampened (30s idle PASS 2026-08-21) — optional longer idle / airplane before formal close. (2) **Ship commercial RC**. (3) **M-OPS-01**. (4) **M-OPS-02**. (5) **M-AUTHZ-02** then **M-AI-01** / Wave 2 as locked. **Customer-managed / BYO storage (`WS-STORAGE`) is Deferred** — must not jump RC/OPS/AI; reopen after `M-DMS-01` starts (`docs/superpowers/plans/2026-08-21-customer-managed-storage-slotting.md`). **Maestro coordination update (2026-08-19):** sim UDID locks are no longer sufficient by themselves; chats must also coordinate `user:*`, `project:*`, and when relevant `task:*` / `title-prefix:*` / `seed:*` / `photos:force-purge` via `scripts/maestro/resource-lock.sh`. **Do not run parallel Maestro against prod until M-DATA-04 is quiet.** **Post-RC idle-parallel:** **M-DATA-03** pull/sync + **M-PERF-01** photo perf + **S-UX-01R**. Evidence: `docs/superpowers/analysis/2026-08-19-photo-sync-resilience-investigation.md`. S-UX-01R plan: `docs/superpowers/plans/2026-08-19-s-ux-01r-retire-add-comment-dead-path.md`. **AI policy:** only high-certainty; abstain otherwise. SoT: `docs/superpowers/plans/2026-08-19-post-rc-boring-loop.md`.
- `WS-OPS / M-OPS-01` **Pipeline**: In-app owner console after RC.
- `WS-OPS / M-OPS-02` **Pipeline**: Shrink hot files + tight states after M-OPS-01.
...
- `WS-AUTHZ / M-AUTHZ-02` **Pipeline (locked 2026-08-21)**: Multi-company project membership — partner liaison + project-scoped invite + optional host-absorb seats. Post-RC after M-OPS-02. Canonical: `documentation/multi-company-project-membership.md`. Clarification addendum in `docs/superpowers/analysis/2026-08-19-roadmap-clarification.md`. Not RC week.
...
- `WS-UX / S-UX-01R` **Pipeline (2026-08-19)**: Retire Add Comment dead path — Update Description is field narrative SoT; delete AddComment screen/nav wiring post-RC. Plan: `docs/superpowers/plans/2026-08-19-s-ux-01r-retire-add-comment-dead-path.md`. Idle-parallel ok.
- `WS-AI / M-AI-01` **Pipeline (2026-08-19)**: On-site Q&A against project tasks/photos/activity. Isolation wall applies. Create Task LLM is on-ramp only.
