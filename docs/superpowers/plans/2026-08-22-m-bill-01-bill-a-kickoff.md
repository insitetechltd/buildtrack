...
Trial (not a tier): customer selects Growth or Unlimited at list price;
  Stripe native trial (trial_end / trial_period_days) on that Price — status trialing.
...
1. **Trial:** **Regular paid `plan_price` + Stripe native trial** (`trial_period_days` or `trial_end` on the chosen Growth/Unlimited Price). Card on file at signup; subscription status `trialing`; auto-charge at list price when trial ends — **same** `locked_plan_price_id`, no SKU change. Trial caps in snapshot; post-trial revision from same locked price via webhook — **not** a separate $0 SKU. Optional coupons are for marketing only, not the standard trial.
