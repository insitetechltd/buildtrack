...
- Current pipeline focus precedence: (1) **`M-OPS-ENV-01` Prod vs Dev Supabase** (empty PROD first — Order 14.94; plan `docs/superpowers/plans/2026-08-26-prod-dev-supabase-split.md`; daily TF→DEV; Stripe live @ App Store submit). (2) Finish **M-BILL-01** on DEV. (3) App Store → live Stripe on PROD. (4) **M-OPS-03** Owner management interface + **M-AUTHZ-02** → **M-AI-01** / Wave 2. **Customer-managed storage (`WS-STORAGE`) Deferred.** Maestro/CI against **DEV only** until PROD is intentional.
...
- `WS-PERF / M-PERF-03` **Pipeline (discussion 2026-08-28; priority TBD)**: Device photo library picker — progressive grid paint, lower-res PhotoKit thumbs, hybrid + Select Photos paths. TF200 defer-pin landed; no design lock. Study: `docs/superpowers/analysis/2026-08-28-device-photo-library-picker-perf-discussion.md`. Distinct from M-PERF-01.
