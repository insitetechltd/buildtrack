...
- Current pipeline focus precedence: (1) **Ship commercial RC**. (2) **M-OPS-01**. (3) **M-OPS-02**. (4) **M-AI-01** high-certainty dataset Q&A. (5) Wave 2 DMS then **M-AI-02** drawing **assist** (cite crop/rev only) then **M-AI-03** barcode spec-check then **M-COST-01**. **Maestro coordination update (2026-08-19):** sim UDID locks are no longer sufficient by themselves; chats must also coordinate `user:*`, `project:*`, and when relevant `task:*` / `title-prefix:*` / `seed:*` / `photos:force-purge` via `scripts/maestro/resource-lock.sh`. **Post-RC idle-parallel (2026-08-19):** **M-DATA-03** pull/sync resilience + **M-PERF-01** evidence photo perf + **S-UX-01R** Add Comment dead-code removal — Pipeline; does not block RC. Evidence: `docs/superpowers/analysis/2026-08-19-photo-sync-resilience-investigation.md`. S-UX-01R plan: `docs/superpowers/plans/2026-08-19-s-ux-01r-retire-add-comment-dead-path.md`. **AI policy:** only high-certainty; abstain otherwise. SoT: `docs/superpowers/plans/2026-08-19-post-rc-boring-loop.md`.
...
- `WS-DATA / M-DATA-03` **Pipeline (2026-08-19)**: Pull/sync resilience — session-gated refresh, retry, honest empty vs error UI, scoped activities fetch. Post-RC; idle-parallel ok.
- `WS-PERF / M-PERF-01` **Pipeline (2026-08-19)**: Evidence photo display perf — disk cache, persisted signed URLs, scoped prefetch. Post-RC; idle-parallel ok.
