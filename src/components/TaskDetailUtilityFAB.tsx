...
export default function TaskDetailUtilityFAB({ onUpdate, onEdit, onCameraUpdate, onCreateSubTask, onCancel, canUpdate, canEdit, canCreateSubTask, canCancel }: TaskDetailUtilityFABProps) {
  ...
  const scaleAnim5 = useRef(new Animated.Value(0)).current;
  ...
  return (
    <>
      ...
      <View className="absolute bottom-6 right-6 items-end" style={{ zIndex: 1000 }}>
        {/* Cancel Task Button */}
        {/* Position: -180px - RED/GRAY for Cancel */}
        {onCancel && (
          <Animated.View
            style={{
              transform: [
                { scale: scaleAnim5 },
                { translateY: scaleAnim5.interpolate({
                  inputRange: [0, 1],
                  outputRange: [0, -180]
                })}
              ],
