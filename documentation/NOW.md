...
**This session — CBP TF 270 scheduled (2026-09-17):** Gate 0 waived (same dogfood class as TF 264–269; Metro→PROD GO on `fa5cff1`). No app leftovers; company landing `docs/company.css` / `docs/index.html` / `docs/company.js` left unstaged. Local `./build-local.sh ios production` → IPA **v1.1.3 (270)** `.eas/artifacts/build-1789660718355.ipa` (CFBundleVersion 270; PROD host `jcnzjigxgkzhjsaekoqz` in `main.jsbundle`; DEV host absent). Bake tip `d9467c4` includes `6184ddd` / `2abf01a` / `fb2d603` / `62b90be`. EAS submit of **this** IPA (not 269): first https://expo.dev/accounts/insitetech/projects/buildtrack/submissions/b4dff77c-09c2-4985-a6ef-6b7849b8c2d7 stalled ~45m on “Submitting”; retry https://expo.dev/accounts/insitetech/projects/buildtrack/submissions/eabc004b-0da3-49d5-9b44-13ec8c89ed6e still polling. **ASC uploaded: NO** as of record — latest listed build remains **269 VALID**. ASC Public / Submit for Review untouched. **Next:** wait EAS→ASC for **270**, then dogfood Select Photos file preview + Joe Assign roster + % dock chrome.

**This session — Cloud Agent sessions dying + company→project wash (2026-09-17):** Cursor Cloud Agent tabs for the company/project landing keep dying (`[internal] internal error` Request ID `95e6b005-783c-4ad8-844d-b51fc02cc9ca`; same class as earlier `3186a36b`). Do not send more into those tabs. Leftover ask continued here: company section now uses `--stage-grad-flip` + `--project-bridge` fade to meet the first work stage. Uncommitted: `docs/company.css`, `docs/index.html` (`company.css?v=23`). **Next:** human glance at company→Amoy scroll; commit if OK. Pages publish needs git push.
