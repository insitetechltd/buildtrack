...
function isDraftTask(task: Task): boolean {
  return String(task.status).toLowerCase() === "draft";
}
...
function deriveInitialQueueFromLegacyFilters(
  ...
  const bucket: TasksQueueBucketId =
    statusFilter === "overdue" || isOverdueFilter(statusFilter)
      ? "overdue"
...
export function useTasksViewAdapter(props?: TasksViewAdapterProps): TasksViewAdapterHookResult {
  ...
  const {
    ...
    const buildRow = (
      task: Task,
      options?: Partial<Pick<TasksScreenRowItem, "queue" | "queueLabel" | "bucket" | "bucketLabel" | "contextLabel">>,
    ): TasksScreenRowItem => {
      ...
      return {
        id: options?.queue && options?.bucket ? `tasks-row:${options.queue}:${options.bucket}:${task.id}` : `tasks-row:draft:${task.id}`,
        taskId: task.id,
        title: task.title,
          cardPresentation: "thumbnail",
        onPress: props?.onNavigateToTaskDetail ? () => props.onNavigateToTaskDetail?.(task.id) : undefined,
        statusToken: isDraftTask(task) ? "custom" : mapTaskStatusToToken(task.status),
        statusLabel: isDraftTask(task) ? "Draft" : formatTaskStatusLabel(task.status),
        ...
        contextLabel: options?.contextLabel ?? projectName,
    ...
    const queuePanels: TasksQueuePanel[] = ([
      ...
      const buckets = (["new", "wip", "review", "overdue"] as TasksQueueBucketId[]).map((bucket) =>
    ...
    const draftsSection: TasksDraftsSection | null = draftRows.length > 0
      ? {
          title: "Drafts",
  ...
  const toggleDraftsSection = () => {
    setIsDraftsExpanded((current) => !current);
  };
