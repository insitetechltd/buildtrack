...
import { useCompanyStore } from '../src/state/companyStore';

/**
 * Script to create admin users for each existing company
 * This will create one admin user per company with appropriate credentials
 */
...
async function createAdminUsers() {
  ...
  if (!supabase) {
    console.error('❌ Supabase not configured. Cannot create users.');
    return;
  }
